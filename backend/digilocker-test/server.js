...
// Enable CORS for all origins (allow React dev server on localhost:3000 to make requests)
app.use(cors());
...
app.get('/api/digilocker/login', (req, res) => {
...
app.get('/api/digilocker/callback', async (req, res) => {
  ...
  try {
    ...
    // TODO: securely store tokens (DB or session)
    res.send(`
...
// Upload to GridFS (memoryStorage)
app.post('/upload/gridfs', uploadMemory.single('file'), (req, res) => {
  ...
  uploadStream.on('error', (err) => {
    ...
    res.status(500).send('Failed to upload to GridFS');
  });
...
// Serve React build files (catch-all route for React)
app.get('*', (req, res) => {
