...
// --- Middleware ---
app.use(cors()); // configure cors if needed for specific origins
...
// --- Multer Setup: store files on disk with unique names ---
...
// JWT Token generation (for demo purpose)
app.post('/auth/token', (req, res) => {
  const { username, password } = req.body;
  // Replace this with proper user DB check in production
...
// Upload file and store in GridFS
app.post('/upload/gridfs', authenticate, upload.single('file'), (req, res) => {
  ...
  fileStream
    ...
    .on('finish', () => {
      // Cleanup local file after upload to GridFS
...
// Download file from GridFS by filename
...
// In-memory user documents (replace with DB in production)
let userDocuments = [];
...
router.post('/create', (req, res) => {
  ...
  // Send the new property back to frontend
